...
TokenStream * init_token_stream(Token * token_stream)
{
    TokenStream * ts = malloc(sizeof(TokenStream));
    ts->token_stream = token_stream;
    return ts;
}
// Remember to free the tokenstream before running this !!!
TokenStream * append_token_string(TokenStream * ts, Token * token)
{
    int i = 0;
    while(ts->token_stream[i].type != END_OF_FILE)
    {
        i++;
    }
    ts->token_stream[i] = *token;
    return ts;
}
